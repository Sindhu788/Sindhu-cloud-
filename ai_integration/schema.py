...
KNOWN_CONDITION_TYPES = ["indicator_compare", "price_compare", "indicator_vs_indicator", "concept", "session", "trend", "raw", "candle_range_pct", "candle_body_pct"]
...
_CONDITION_SCHEMA_NOTE = """Each condition object: {"type": one of indicator_compare|price_compare|indicator_vs_indicator|concept|session|trend|candle_range_pct|candle_body_pct|raw, "indicator": indicator name (for indicator_compare/price_compare/indicator_vs_indicator -- for indicator_vs_indicator this is the FIRST indicator), "params": {"period": N} if applicable else {} (for indicator_vs_indicator, this is the first indicator's params; for candle_range_pct, {"min_pct": N, "max_pct": N} -- either may be omitted to leave that side unbounded; for candle_body_pct, {"min_pct": N} -- omit/null if the document doesn't give a specific percentage), "op": ">" or "<" only, "value": number (REQUIRED for indicator_compare -- compares the indicator to this fixed number; leave null/omit for price_compare and indicator_vs_indicator, which never use it), "indicator2": ONLY for type="indicator_vs_indicator" -- the SECOND indicator's name, "params2": ONLY for type="indicator_vs_indicator" -- the second indicator's params (e.g. {"period": 50}), "name": concept/session name (for concept/session types), "direction": "bullish"|"bearish"|null (for concept/trend types), "text": the original phrase (required when type="raw" -- use raw ONLY when you cannot express the rule with the vocabulary below), "role": for "concept" AND "indicator_vs_indicator" conditions, the timeframe role (one of the declared Timeframe roles below, e.g. "bias"/"trend"/"analysis") that THIS SPECIFIC comparison is actually read from -- for indicator_vs_indicator BOTH indicators come from this SAME role (e.g. "Trend (1H): 20 EMA above 50 EMA" -> role="trend", both EMAs are the 1H versions); set it whenever the text names a timeframe for it (e.g. "a swing low on the 1H or 4H chart" -> role of whichever declared role is 1h or 4h); leave null when the text doesn't say (this defaults to the entry timeframe, which is correct for most entry/confirmation-timeframe rules like a 1-minute candle_break trigger) -- never leave every condition on the entry timeframe just because that's simpler than reading which chart the text actually says it's on, "lookback_bars": null (leave null).
...
USE type="candle_body_pct" whenever the document requires a "strong"/"decisive" candle or warns against "small"/"Doji" candles (a candle-BODY-strength quality filter, distinct from candle_range_pct's overall size filter) -- e.g. "a valid sweep should occur with strong candles", "avoid Doji candles". If no specific percentage is given, still use this type with params.min_pct left null rather than falling back to raw/not-a-rule -- this correctly flags the threshold itself as the thing that still needs a human's input, without losing the fact that a real candle-quality rule exists."""
...
def build_structured_extraction_prompt(source_hint=None, content_type=None):
    ...
    return (
        ...
        "NOT a condition\n"
        "  * a session-scoping rule restricting WHEN the strategy trades "
        "(\"only during the New York session\", \"NY session\", \"trades "
        "during London\", \"London open\", \"Asian session only\") -> add the "
        "matching name (\"ny\", \"london\", or \"asian\") to the top-level "
        "\"session_filter\" list -- NOT a condition, and never dropped just "
        "because it's stated once for a whole model rather than per-rule\n"
        ...
        "then it IS X -- emit X. But a vague resemblance is NOT agreement: a "
        "Low Volume Node is NOT \"support\" merely because both are price "
        "levels, and \"Order Flow Aggression\" is NOT \"liquidity_sweep\" "
        "merely because both involve large orders -- do not force a foreign "
        "mechanism onto a superficially-similar known concept just to avoid "
        "raw. Known volume-profile-family equivalents that DO exist, so map "
        "these directly: \"Volume Profile POC\"/\"Point of Control\" -> "
        "concept \"poc\"; \"Value Area\" -> concept \"value_area\"; \"Low "
        "Volume Node\"/\"LVN\" -> concept \"lvn\"; \"High Volume Node\"/\"HVN\" "
        "-> concept \"hvn\"; \"Order Flow Aggression\"/\"aggressive orders\"/"
        "\"big print\" -> concept \"aggression\"; \"Cumulative Volume Delta\"/"
        "\"CVD\" -> indicator \"cvd\".\n\n"
...
def build_statement_extraction_prompt(source_hint=None, content_type=None):
    ...
    return (
        ...
        "trading logic.\n\n"
        "DO NOT SUBSTITUTE A DIFFERENT MECHANISM JUST TO AVOID RAW. A vague "
        "resemblance is not a match -- only commit to a vocabulary term when "
        "the underlying MECHANISM is genuinely the same, not just superficially "
        "similar (e.g. a Low Volume Node is NOT \"support\" merely because both "
        "are price levels; \"Order Flow Aggression\" is NOT \"liquidity_sweep\" "
        "merely because both involve large orders). If the statement names a "
        "specific mechanism/indicator with no real equivalent below, use "
        "type=\"raw\" with the exact text -- this is the SAFE choice, never a "
        "failure. Known volume-profile-family equivalents that DO exist, so "
        "map these directly rather than raw: \"Volume Profile POC\"/\"Point of "
        "Control\" -> concept \"poc\"; \"Value Area\" -> concept \"value_area\"; "
        "\"Low Volume Node\"/\"LVN\" -> concept \"lvn\"; \"High Volume Node\"/"
        "\"HVN\" -> concept \"hvn\"; \"Order Flow Aggression\"/\"aggressive "
        "orders\"/\"big print\" -> concept \"aggression\"; \"Cumulative Volume "
        "Delta\"/\"CVD\" -> indicator \"cvd\".\n\n"
        "A statement that ONLY names which chart/timeframe to use for "
        "something (\"use the 4-hour chart to mark your key levels\", "
        "\"use the 5-minute chart for entering and managing trades\") is a "
        "TIMEFRAME declaration, not a condition -- set the matching key in "
        "the top-level \"timeframes\" object (role -> interval, e.g. "
        "{\"analysis\": \"4h\", \"entry\": \"5m\"}) and leave "
        "entry_conditions/exit_conditions/etc. empty for that statement. "
        "Only use a condition type when the statement ALSO states a "
        "concrete comparison/trigger, not just which chart to look at.\n\n"
        "A STOP-LOSS anchored to the specific TRIGGER/SIGNAL CANDLE itself "
        "(\"place the stop slightly above/below the high/low created by "
        "the sweep\", \"stop-loss = the signal candle's high * 1.003\") "
        "uses stop_loss type=\"signal_candle\", never \"structure\" or "
        "\"level\" -- those mean a swing/order-block/FVG zone or a named "
        "level like PDH/PDL, not the specific candle the entry itself "
        "fired on. \"value\" is the buffer as an ordinary percent number "
        "(1.0 means 1%). If the source text doesn't give a specific "
        "buffer number (\"slightly above\", \"a small margin\"), still use "
        "type=\"signal_candle\" with value left null -- do not guess a "
        "number, and do not fall back to type=\"level\"/\"structure\" just "
        "because no number was given.\n\n"
        "A CANDLE-QUALITY filter (\"a valid sweep should occur with strong "
        "candles\", \"avoid small or Doji candles\") is a real, executable "
        "rule -- map it to type=\"candle_body_pct\" (a bar's body as a "
        "percent of its own high-low range). If the statement gives no "
        "specific percentage, still use is_rule=true and this type, with "
        "params.min_pct left null (never invent a number) -- do NOT mark "
        "it not_a_rule just because a threshold isn't quantified.\n\n"
        "GENERAL ADVICE IS NOT A RULE: a sentence that gives general "
        "trading discipline/mindset advice with no specific, checkable "
        "condition of its own (\"be patient and wait for the best setup\", "
        "\"apply the strategy consistently to build confidence\", \"keep "
        "your chart markings up to date as new levels form\") is "
        "commentary, not an executable rule, even if it uses words like "
        "\"until\"/\"if\"/a price level's name -- is_rule=false for these. "
        "The test: could this sentence alone be checked true/false against "
        "one bar of price data? If not, it is commentary.\n\n"
        ...
        "top-level day_filter list, not a condition. SESSION-SCOPING LANGUAGE: "
        "any statement restricting trading to a specific session (\"New York "
        "session only\", \"NY session\", \"during London\", \"London open\", "
        "\"Asian session\", \"trades only in the London session\") must add "
        "the matching name (\"ny\", \"london\", or \"asian\") to the top-level "
        "session_filter list -- never drop it and never express it as a "
        "condition instead.\n\n"

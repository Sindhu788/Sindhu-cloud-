"""Quick single-symbol (BTCUSDT) sanity check for all 8 New Batch 5
Strategy 9 combinations before committing to the full 50-coin backtests."""
import sys
import os
import json
...
with open(os.path.join(os.path.dirname(__file__), "..", "data", "checkpoints", "_strategy9_ids.json")) as f:
    ids = json.load(f)
...
for variant, sid in ids.items():
    ...
    for name, sec in report["sections"].items():
        if sec["status"] == "FAIL":
            print(f"  {name}: FAIL --", json.dumps({k: v for k, v in sec.items() if k not in ("metrics",)}, default=str)[:600])

"""Quick single-symbol (BTCUSDT) sanity check for New Batch 5 Strategy 1
before committing to the full 50-coin backtest -- same technique
scripts/run_all_ready_backtests.py already uses (Engine Health Report:
Strategy/Data/Execution/PnL/Trade/Statistics verification)."""
import sys
import os
import json
...
for variant, sid in ids.items():
    ...
    for name, sec in report["sections"].items():
        ...
        if sec["status"] == "FAIL":
            print("    detail:", json.dumps({k: v for k, v in sec.items() if k not in ("metrics",)}, default=str)[:800])

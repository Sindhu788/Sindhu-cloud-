...
def telegram_network_check():
    ...
    _LOGIN_EXEMPT_PATHS) -- same reasoning as /health: a real, credential-
    independent outbound reachability test to api.telegram.org from
    wherever THIS process is actually running, so "can Telegram be
    reached from here" can be checked directly (e.g. via curl) without a
    browser session, and without needing a bot token/channel id to be
...
@router.get("/api/paper-trading/_diag/phase1-report")
def _diag_phase1_report():
    """TEMPORARY (2026-09-13) -- Phase 1 diagnosis of the 5-phase improvement
    batch. Read-only, no side effects, no credentials touched. Pulls real
    production evidence for: (1) the Telegram send-pipeline trace, reusing
    the already-existing, always-on near_miss_log and generated-signal
    delivery log tables (no new tracing added), (2) Evolution engine's real
    generation-event count from evolution_comparisons, (3) fee-adjusted vs
    raw PnL on real closed trades, (4) profit concentration across real
    strategies. To be removed (this route + its security.py exemption)
    once its output has been captured -- same temporary-diagnostic pattern
    used for the one-time PnL report earlier this project."""
    settings = telegram_bot.load_settings()

    since_iso = (datetime.now(timezone.utc) - timedelta(days=14)).isoformat()
    near_misses = storage.list_near_misses(limit=30)
    generated = storage.list_generated_signals_with_delivery(since_iso=since_iso)[:30]

    evo_error = None
    try:
        evo_all = storage.list_evolution_comparisons(limit=100000)
    except Exception as e:
        evo_all = None
        evo_error = repr(e)

    closed = storage.list_closed_paper_positions(limit=5000)
    commission_pct = 0.001  # 0.1% round trip -- matches this project's own backtest default (commission_pct=0.1)
    raw_total = 0.0
    fee_adjusted_total = 0.0
    total_commission = 0.0
    per_strategy = {}
    for p in closed:
        pnl = p.get("pnl") or 0.0
        notional = (p.get("entry_price") or 0.0) * (p.get("size") or 0.0)
        commission = notional * commission_pct
        raw_total += pnl
        fee_adjusted_total += (pnl - commission)
        total_commission += commission
        key = p.get("strategy_id") or "__lessons__"
        bucket = per_strategy.setdefault(key, {
            "strategy_id": key, "strategy_name": p.get("strategy_name") or "(no strategy)",
            "raw_pnl": 0.0, "fee_adjusted_pnl": 0.0, "trades": 0, "wins": 0,
        })
        bucket["raw_pnl"] += pnl
        bucket["fee_adjusted_pnl"] += (pnl - commission)
        bucket["trades"] += 1
        if pnl > 0:
            bucket["wins"] += 1

    ranked = sorted(per_strategy.values(), key=lambda v: v["raw_pnl"], reverse=True)
    positive_total = sum(v["raw_pnl"] for v in ranked if v["raw_pnl"] > 0)

    return {
        "settings_snapshot": {
            "auto_send_enabled": settings.get("auto_send_enabled"),
            "master_send_enabled": settings.get("master_send_enabled"),
            "auto_send_high_confidence_only": settings.get("auto_send_high_confidence_only"),
            "auto_send_min_confluence_ratio": settings.get("auto_send_min_confluence_ratio"),
            "auto_send_min_confluence_count": settings.get("auto_send_min_confluence_count"),
            "rate_limit_per_hour": settings.get("rate_limit_per_hour"),
            "signal_freshness_minutes": settings.get("signal_freshness_minutes"),
        },
        "telegram_trace": {
            "near_miss_total_returned": len(near_misses),
            "near_misses": near_misses,
            "recent_generated_signals_since": since_iso,
            "recent_generated_signals_count": len(generated),
            "recent_generated_signals": generated,
        },
        "evolution": {
            "error": evo_error,
            "total_comparisons_all_time": len(evo_all) if evo_all is not None else None,
            "recent_comparisons": evo_all[:10] if evo_all else [],
        },
        "pnl_fee_adjustment": {
            "closed_trades_counted": len(closed),
            "commission_pct_assumed_round_trip": commission_pct * 100,
            "raw_total_pnl": round(raw_total, 2),
            "fee_adjusted_total_pnl": round(fee_adjusted_total, 2),
            "total_commission_deducted": round(total_commission, 2),
        },
        "profit_concentration": {
            "strategy_count_with_closed_trades": len(per_strategy),
            "strategies_with_positive_raw_pnl": sum(1 for v in ranked if v["raw_pnl"] > 0),
            "total_positive_pnl_from_winning_strategies": round(positive_total, 2),
